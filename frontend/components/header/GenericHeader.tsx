      ...
      try {
        if (role === "professional") {
          const res = await fetch(
            `${process.env.NEXT_PUBLIC_API_URL}/notifications?sellerId=${userData.id}`
          );
          if (res.ok) {
          ...
          const res = await fetch(
            `${process.env.NEXT_PUBLIC_API_URL}/messages/user/${userData.id}`
          );
          if (res.ok) {
            const msgs = await res.json();
            const unread = Array.isArray(msgs)
              ? msgs.filter((m: any) => !m.is_read).length
              : 0;
      ...
      try {
        if (role === "professional") {
          const [offersRes, messagesRes] = await Promise.all([
            fetch(
              `${process.env.NEXT_PUBLIC_API_URL}/offers/seller/${userData.id}`,
              { headers }
            ),
            fetch(
              `${process.env.NEXT_PUBLIC_API_URL}/messages/user/${userData.id}`,
          ...
          const oItems = Array.isArray(offers)
            ? offers.map((o: any) => ({
                type: "offer",
                id: o.id,
                item_id: o.item_id,
            ...
            : [];
          const mItems = Array.isArray(messages)
            ? messages.map((m: any) => {
          ...
          const [offersRes, messagesRes] = await Promise.all([
            fetch(
              `${process.env.NEXT_PUBLIC_API_URL}/offers/buyer/${userData.id}`,
              { headers }
            ),
            fetch(
              `${process.env.NEXT_PUBLIC_API_URL}/messages/user/${userData.id}`,
