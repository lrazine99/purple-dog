...
const ORDER_STATUSES = [
  { value: "draft", label: "Brouillon", color: "bg-slate-500/20 text-slate-400", icon: Clock },
    ...
    <div className="relative">
      <div className="relative">
        ...
        <input
          ...
          className={`w-full h-10 pl-10 pr-4 bg-slate-900/50 border border-slate-700 rounded-lg text-white placeholder:text-slate-500 ${colors.focus} focus:outline-none`}
        />
      </div>
    ...
    <div className="relative flex-1">
      <div className="relative">
        ...
        <input
          ...
          className="w-full h-10 pl-10 pr-4 bg-slate-900/50 border border-slate-700 rounded-lg text-white placeholder:text-slate-500 focus:border-green-500 focus:outline-none"
        />
      </div>
    ...
    <div className="space-y-6">
      ...
      <div className="relative">
        ...
        <Input
          ...
          className="h-12 pl-12 bg-slate-800/50 border-slate-700 text-white placeholder:text-slate-500 focus:border-green-500"
        />
      </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-slate-900/50 border-b border-slate-700">
                <tr>
                  <th className="text-left px-6 py-4 text-slate-400 font-medium">ID</th>
                  <th className="text-left px-6 py-4 text-slate-400 font-medium">Acheteur</th>
                  <th className="text-left px-6 py-4 text-slate-400 font-medium">Vendeur</th>
                  <th className="text-left px-6 py-4 text-slate-400 font-medium">Total</th>
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-white">{getUserName(order.buyer)}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2">
                          <Store className="w-4 h-4 text-purple-400" />
                          <span className="text-white">{getUserName(order.seller)}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4">
      ...
      {selectedOrder && (
        <div className="fixed inset-y-0 right-0 w-96 bg-slate-800 border-l border-slate-700 shadow-2xl z-40 overflow-y-auto">
          <div className="p-6">
            ...
            <div className="mb-6">
              ...
              <select
                ...
                className="w-full h-10 px-3 bg-slate-900/50 border border-slate-700 rounded-lg text-white focus:border-green-500 focus:outline-none"
              >
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              <div className="p-4 bg-slate-900/50 rounded-xl">
                <p className="text-slate-400 text-xs mb-1">Acheteur</p>
                <p className="text-white font-medium">{getUserName(selectedOrder.buyer)}</p>
                <p className="text-slate-400 text-sm">{selectedOrder.buyer?.email}</p>
              </div>
              <div className="p-4 bg-slate-900/50 rounded-xl">
                <p className="text-slate-400 text-xs mb-1">Vendeur</p>
                <p className="text-white font-medium">{getUserName(selectedOrder.seller)}</p>
                <p className="text-slate-400 text-sm">{selectedOrder.seller?.email}</p>
              </div>
            </div>
            ...
            {/* Items */}
            <div className="mb-6">
              <h3 className="text-sm font-medium text-slate-400 mb-3">Articles ({selectedOrder.items?.length || 0})</h3>
              <div className="space-y-2">
                {selectedOrder.items?.map((item) => (
                  <div key={item.id} className="flex items-center justify-between p-3 bg-slate-900/50 rounded-lg">
                    <div>
                      <p className="text-white">{item.item?.name || `Article #${item.item_id}`}</p>
                      <p className="text-slate-400 text-sm">Qté: {item.qty}</p>
                    </div>
                    <p className="text-green-400 font-medium">{item.unit_price} €</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="bg-slate-800 border border-slate-700 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-slate-300 mb-2">Acheteur</label>
                  <UserAutocomplete
                    users={users}
                ...
                <div>
                  <label className="block text-sm font-medium text-slate-300 mb-2">Vendeur</label>
                  <UserAutocomplete
                    users={users}
              ...
              {!editingOrder && (
                <div>
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <Button onClick={addItemToOrder} size="sm" variant="outline" className="bg-slate-700 border-slate-600">
                  ...
                  <div className="space-y-3">
                    {form.items.map((orderItem, index) => (
                      <div key={index} className="flex gap-3 items-center p-3 bg-slate-900/30 rounded-xl">
                        <ItemAutocomplete
                          items={items}
                        ...
                        />
                        <div className="flex items-center gap-2">
                          <span className="text-slate-400 text-sm">Qté:</span>
                          <Input
                            type="number"
                            min="1"
                            value={orderItem.qty}
                            onChange={(e) => updateOrderItemQty(index, parseInt(e.target.value) || 1)}
                            className="w-20 bg-slate-900/50 border-slate-700 text-white text-center"
                          />
                        </div>
                        {orderItem.item && (
